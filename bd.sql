...
CREATE TABLE IF NOT EXISTS `bd`.`productos` (
  ...
  `categ_prod_id_cat_prod` INT NOT NULL,
  ...
  INDEX `fk_productos_categ_prod1_idx` (`categ_prod_id_cat_prod` ASC) VISIBLE,
  CONSTRAINT `id_carter`
  ...
  CONSTRAINT `fk_productos_usuarios1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_productos_categ_prod1`
    FOREIGN KEY (`categ_prod_id_cat_prod`)
...
CREATE TABLE IF NOT EXISTS `bd`.`locales` (
  ...
  CONSTRAINT `id_prod_locales`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `dia_hor_local`
    FOREIGN KEY (`dia_hor_local`)
    REFERENCES `bd`.`horarios_local` (`dia_hor_local`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bd`.`orden_detalle` (
  `id_ord_deta` INT NOT NULL,
  `orden_id_orden` INT NOT NULL,
  `productos_id_prod` INT NOT NULL,
  PRIMARY KEY (`id_ord_deta`),
  INDEX `fk_orden_detalle_orden1_idx` (`orden_id_orden` ASC) VISIBLE,
  INDEX `fk_orden_detalle_productos1_idx` (`productos_id_prod` ASC) VISIBLE,
  CONSTRAINT `fk_orden_detalle_orden1`
    FOREIGN KEY (`orden_id_orden`)
    REFERENCES `bd`.`orden_compra` (`id_orden`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_orden_detalle_productos1`
